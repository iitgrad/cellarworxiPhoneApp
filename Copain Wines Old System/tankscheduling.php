...
$start_day=$today-(86400*$range);



/*$query='SELECT DISTINCT assets.ID, reservation.WOID, `assets`.`NAME`, wo.CLIENTCODE, wo.LOT, wo.TYPE, assets.OWNER,

        `assets`.`CAPACITY`, UNIX_TIMESTAMP(`wo`.`DUEDATE`) AS STARTDATE,

          UNIX_TIMESTAMP(`wo`.`ENDDATE`) AS THEENDDATE

       FROM  `assets` 

           LEFT OUTER JOIN `reservation` ON (`assets`.`ID` = `reservation`.`ASSETID`)
       ...
       WHERE
           ...
           AND (not isnull(wo.DUEDATE))

           AND (not isnull(wo.ENDDATE))

           AND (assets.HIDDEN="NO")
...

{
...

	{
		...
		//       echo $row['ID'].' '.strftime("%m",$row['STARTDATE']).'/'.strftime("%d",$row['STARTDATE']).'/'.strftime("%Y",$row['STARTDATE']).'<br>';
...
//				$tank[$index]['val'][$j]='<a  href="scppage.php?action=view&woid='.$row['WOID'].'" onmouseover="return overlib(\''.$data.'\',
				...
				$tank[$index]['val'][$j]='<a  href="scppage.php?action=view&woid='.$row['WOID'].'" onmouseover="return overlib(\''.''.'\',
...

				{
					...
					$tank[$index]['val'][$j+$allocindex].='<a  href="scppage.php?action=view&woid='.$row['WOID'].'" onmouseover="return overlib(\''.''.'\',
