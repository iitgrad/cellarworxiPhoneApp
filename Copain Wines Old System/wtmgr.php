...
$query = 'SELECT DISTINCT
  ...
  SUM(`bindetail`.`WEIGHT`) AS `TOTALWEIGHT`,

  SUM(`bindetail`.`TARE`) AS `TOTALTARE`

FROM

  `wt`

  INNER JOIN `bindetail` ON (`wt`.`TAGID` = `bindetail`.`WEIGHTAG`)
...
WHERE
  ...
  echo ' <font face="Franklin Gothic Book" size="2"><table width="100%" border="1" bordercolor="#111111">

    <tr><td width="100%" align="center" colspan="3"><b>';

       echo $_GET['lot'].'  '.$_GET['vesseltype'].'  '.$_GET['vessel'].'
    ...
    <tr>
     ...
     echo '<form method="POST" action=viewfermcurves.php?lot='.
           ...
           echo        '<input type="text" name="Date" size="10" tabindex="1">';
