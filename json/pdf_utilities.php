...
function gen_topping_task($pdf, $taskid)
{
...
//	$pdf->SetLeftMargin(5);
  	$pdf->SetFont('Arial','B',8);
   	...
   	$pdf->Cell(100,3,"COPAIN CUSTOM CRUSH",0,0,"L",0);
  	$pdf->SetFont('Arial','',8);
	$pdf->Cell(30,3,"Dates of Work:",0,0,"L",0);
	$pdf->Cell(30,3,date("m/d/Y",strtotime($data['startdate'])).' - '.date("m/d/Y",strtotime($data['enddate'])),0,1,"L",0);

   	$pdf->Cell(15,3,"Task ID:",0,0,"L",0);
   	$pdf->Cell(85,3,$data['taskid'],0,0,"L",0);
   	$pdf->Cell(30,3,"Work Performed By:",0,0,"L",0);
   	$pdf->Cell(30,3,$data['workperformedby'],0,1,"L",0);

   	$pdf->Cell(15,3,"Winery:",0,0,"L",0);
   	$pdf->Cell(85,3,$data['wos'][0]['CLIENTNAME'],0,0,"L",0);
   	$pdf->Cell(30,3,"Submission Date:",0,0,"L",0);
   	$pdf->Cell(30,3,$data['wos'][0]['CREATIONDATE'],0,1,"L",0);

   	$pdf->Cell(15,3,"Activity:",0,0,"L",0);
   	$pdf->Cell(85,3,$data['type'],0,0,"L",0);
   	$pdf->Cell(30,3,"Requestor:",0,0,"L",0);
   	$pdf->Cell(30,3,$data['wos'][0]['REQUESTOR'],0,1,"L",0);

   	$pdf->Cell(25,3,"",0,1,"L",0); //Blank Line

	$headingHeight=12;
	$lineHeight=4;
	
   	$pdf->Cell(25,$headingHeight,"Lot",1,0,"C",0);
   	$pdf->Cell(60,$headingHeight,"Description",1,0,"C",0);
   	$pdf->Cell(15,$headingHeight,"Gal",1,0,"C",0);

	$x=$pdf->GetX();
	$y=$pdf->GetY();
   	$pdf->MultiCell(15,$lineHeight,"BBL's Before",1,0,"C",0);
	$pdf->SetXY($x+15,$y);
  
	$x=$pdf->GetX();
	$y=$pdf->GetY();
   	$pdf->MultiCell(15,$lineHeight,"BBL's After",1,0,"C",0);
	$pdf->SetXY($x+15,$y);

	$x=$pdf->GetX();
	$y=$pdf->GetY();
   	$pdf->MultiCell(15,$lineHeight,"Topping Mat'l Before",1,0,"C",0);
	$pdf->SetXY($x+15,$y);

	$x=$pdf->GetX();
	$y=$pdf->GetY();
   	$pdf->MultiCell(15,$lineHeight,"Topping Mat'l After",1,0,"C",0);
	$pdf->SetXY($x+15,$y);

   	$pdf->Cell(20,$headingHeight,"Top With",1,0,"C",0);

	$x=$pdf->GetX();
	$y=$pdf->GetY();
   	$pdf->MultiCell(14,$lineHeight,"SO2 Add per BBL (mL)",1,0,"L",0);
	$pdf->SetXY($x+14,$y);
   	$pdf->Cell(60,$headingHeight,"Notes",1,1,"C",0);	
	$pdf->setY($pdf->getY()+8);
	...
	for ($i=0;$i<count($data['wos']);$i++)
	{
		...
		$lotinfo=$data['lotinfo'][$i];
	   	$pdf->Cell(25,4,$wo['LOT'],1,0,"C",0);		
//	   	$pdf->Cell(60,4,$wo['LOTDESCRIPTION'],1,0,"C",0);		
		$x=$pdf->GetX();
		$y=$pdf->GetY();
	   	$pdf->MultiCell(60,$lineHeight,$wo['LOTDESCRIPTION'],1,0,"L",0);		
		$pdf->SetXY($x+60,$y);
	   	$pdf->Cell(15,4,$lotinfo[count($lotinfo)-1]['ending_tankgallons']+$lotinfo[count($lotinfo)-1]['ending_bbls']*60+$lotinfo[count($lotinfo)-1]['ending_toppinggallons'],1,0,"C",0);		
	   	$pdf->Cell(15,4,$lotinfo[count($lotinfo)-1]['ending_bbls'],1,0,"C",0);		
	   	$pdf->Cell(15,4,'',1,0,"C",0);		
	   	$pdf->Cell(15,4,$lotinfo[count($lotinfo)-1]['ending_toppinggallons'],1,0,"C",0);		
	   	$pdf->Cell(15,4,'',1,0,"C",0);		
	   	$pdf->Cell(20,4,$wo['TOPPINGLOT'],1,0,"C",0);		
	   	$pdf->Cell(14,4,$so2,1,0,"C",0);		
	   	$pdf->MultiCell(60,4,$wo['OTHERDESC'],1,1,"C",0);		
	}
	...
	$pdf->Cell(50,6,'Completion Comments:',0,1);
//	$pdf->SetX($x+1);

	$pdf->Cell(30,4,'Work Performed By:',0,0);
	$pdf->Cell(20,4,'__________________',0,0);

	$pdf->Cell(80,4,'',0,0);
	$pdf->Cell(30,4,'Completion Date:',0,0);
	$pdf->Cell(20,4,'__________________',0,0);

	// $pdf->SetX($x-1);
	// $pdf->SetY($y+33);
	// $pdf->Cell(30,4,'Notes:',0,1);
	// $pdf->Cell(30,4,'If we are not topping "Top With" field says "DO NOT TOP":',0,1);
...
function theheader($pdf)
{
    ...
    $pdf->ln(9);
    $pdf->Cell(195,7,'COPAIN CUSTOM CRUSH',0,1,"C",0);
...
function gen_popd_matrix($pdf)
{
  ...
  $query='select type,vesseltype,vesselid,lot, dryice, stattemp, timeslot,strength,duration,lots.description,clients.group from wo left outer join lots on (lots.LOTNUMBER=wo.LOT) left outer join clients on (wo.CLIENTCODE=clients.CODE) where duedate=curdate() and wo.DELETED!=1 
		and (TYPE="PUMP OVER" or wo.TYPE="PUNCH DOWN") order by clients.group, wo.vesseltype, vesselid';
...
//	echo $query; exit;
	...
	for ($i=0; $i<mysql_num_rows($result); $i++)
	{
		...
		$vessel=$row['vesseltype'].'-'.$row['vesselid']." ".$row['lot'];
	...
	foreach ($action as $group=>$data)
	{
		...
		$pdf->ln(2);
		$pdf->Cell(30,7,"VESSEL & LOT",1,0,"C",0);
//		$pdf->Cell(15,7,"LOT",1,0,"C",0);
		...
		{
			$pdf->Cell(30,7,$thevessel,1,0,"C",0);
